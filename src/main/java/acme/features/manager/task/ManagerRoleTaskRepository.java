...
@Repository
public interface ManagerRoleTaskRepository extends AbstractRepository {

	@Query("select ct from Task ct where ct.owner.id = :id")
	Collection<Task> findManagerRoleTaskById(@Param("id") int id);

	@Query("select t from Task t where t.id = :id")
	Task findOneTaskById(@Param("id") int id);
	
	@Query("select m from ManagerRole m where m.id = :id")
	ManagerRole findOneManagerRoleById(@Param("id") int id);
